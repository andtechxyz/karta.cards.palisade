...
public final class FIDO2Applet extends Applet implements ExtendedLength {
    ...
    // -----------------------------------------------------------------------
    // T4T SIO integration — RP ID triggers URL update on PalisadeT4T
    // -----------------------------------------------------------------------
    private static final byte[] T4T_AID = {
        (byte) 0xD2, (byte) 0x76, (byte) 0x00, (byte) 0x00,
        (byte) 0x85, (byte) 0x01, (byte) 0x01
    };
    private static final byte T4T_STATE_SHIPPED = 0x00;
    private static final byte T4T_STATE_ACTIVATED = 0x01;
    private static final short T4T_RPID_BUF_LEN = (short) 150;
    /** Persistent: card activation state. */
    private byte[] t4tCardState; // t4tCardState[0] = T4T_STATE_SHIPPED or T4T_STATE_ACTIVATED
    /** Persistent: saved RP ID for SIO URL update (copied before buffer gets overwritten). */
    private byte[] t4tSavedRpId;
    private short t4tSavedRpIdLen;
    /**
     * Activation URL — stored during personalization via SET_ACTIVATION_URL (0x80 F2).
     * Passed to T4T's setUrl() on first U2F Authenticate (U2F only has RP ID hash).
     */
    private byte[] t4tActivationUrl;
    private short t4tActivationUrlLen;
    /**
    ...
    private void makeCredential(APDU apdu, short lc, byte[] buffer) {
        ...
        // (Earlier versions of this applet copied the RP ID into t4tSavedRpId
        // here so that makeCredential's epilogue could push it into the T4T
        // applet as the activation URL.  Path A delivers the activation URL
        // through getAssertion's `karta-url` extension instead, so this copy
        // is no longer needed.  Field is left in place for backwards
        ...
        // T4T: makeCredential intentionally does NOT activate the T4T applet.
        //
        // Pre-registered creds (perso-time mint, before the card ships) must
        // leave the chip in SHIPPED state so the cardholder's first tap drives
        // the SUN-pre-activation URL (e.g. tap.karta.cards/activate/<cardRef>).
        // Activation happens later, during getAssertion, when the karta-url
        // WebAuthn extension or the extended-cred-ID tail delivers the post-
        // activation URL + CMAC and we call setUrlWithMac() via SIO.
        //
        // (Earlier versions of this applet auto-activated here; that path is
        // gone now that Path A delivers the activation payload through
        // getAssertion instead of relying on perso-time activation.)
    ...
    private void getAssertion(final APDU apdu, final short lc, final byte[] buffer,
        ...
        if (firstCredIdx == 0) { // Start of iteration: actually parse request
                    ...
                    case 0x04: // extensions
                        ...
                        for (j = 0; j < mapEntries; j++) {
                            ...
                            if (keyLength == CannedCBOR.CRED_BLOB_EXTENSION_ID.length && Util.arrayCompare(buffer, readIdx,
                                ...
                                continue;
                            } else if (keyLength == CannedCBOR.KARTA_URL_EXTENSION_ID.length && Util.arrayCompare(buffer, readIdx,
                                    CannedCBOR.KARTA_URL_EXTENSION_ID, (short) 0, (short) CannedCBOR.KARTA_URL_EXTENSION_ID.length) == 0) {
                                // karta.cards extension — byte-string value
                                // holds <url || cmac(16)>.  Buffer it now;
                                // the SIO block after the assertion is
                                // signed will push it to the T4T applet
                                // (same path u2FAuthenticate takes for the
                                // CTAP1 extended-credential-ID tail).
                                readIdx += keyLength;
                                short kartaValLen;
                                byte kartaHdr = buffer[readIdx++];
                                if (kartaHdr == 0x58) {
                                    // 1-byte length follows
                                    kartaValLen = (short)(buffer[readIdx++] & 0xFF);
                                } else if ((kartaHdr & 0xE0) == 0x40) {
                                    // immediate length 0..23 in low 5 bits
                                    kartaValLen = (short)(kartaHdr & 0x1F);
                                } else {
                                    sendErrorByte(apdu, FIDOConstants.CTAP2_ERR_CBOR_UNEXPECTED_TYPE);
                                    return;
                                }
                                // Minimum meaningful payload is 1 url byte
                                // + 16 CMAC bytes; upper bound is our SIO
                                // buffer size.  Anything outside that
                                // window we silently drop — the assertion
                                // itself still goes through.
                                if (kartaValLen > (short) 16 && kartaValLen <= T4T_RPID_BUF_LEN) {
                                    Util.arrayCopyNonAtomic(buffer, readIdx,
                                            t4tActivationUrl, (short) 0, kartaValLen);
                                    t4tActivationUrlLen = kartaValLen;
                                }
                                readIdx += kartaValLen;
                                continue;
            ...
            if (allowListIdx != -1) {
                ...
                for (short i = 0; i < allowListLength; i++) {
                    ...
                    // getStoredLen() returns a signed byte — lengths > 127
                    // sign-extend to negative shorts.  Mask to 8 unsigned bits.
                    short credLen = (short)(transientStorage.getStoredLen() & 0xFF);

                    // karta.cards extension: the server may append a
                    // URL + CMAC tail to the credential ID to piggy-back a
                    // chip URL update onto the assertion (mirrors what
                    // u2FAuthenticate already does).  Tail layout:
                    //   <real_credential_id(CREDENTIAL_ID_LEN)> || <url(N)> || <cmac(16)>
                    // The minimum tail is 1 byte URL + 16 byte CMAC = 17
                    // bytes, so any credId longer than CREDENTIAL_ID_LEN by
                    // at least 17 is treated as extended.
                    if (credLen > (short)(CREDENTIAL_ID_LEN + 16)) {
                        short tailLen = (short)(credLen - CREDENTIAL_ID_LEN);
                        if (tailLen <= T4T_RPID_BUF_LEN) {
                            Util.arrayCopyNonAtomic(buffer,
                                    (short)(credIdx + CREDENTIAL_ID_LEN),
                                    t4tActivationUrl, (short) 0, tailLen);
                            t4tActivationUrlLen = tailLen;
                        }
                        credLen = CREDENTIAL_ID_LEN;
                        // Update transient storage so downstream code sees
                        // the real credential length.  CREDENTIAL_ID_LEN
                        // (112) fits in a signed byte so no mask needed.
                        transientStorage.setStoredVars(credIdx, (byte) CREDENTIAL_ID_LEN);
                    }
        ...
        // karta.cards extension: if the allowList credential ID OR the
        // `karta-url` extension delivered a URL + CMAC blob, now is the time
        // to push that to the T4T applet via SIO — mirrors what
        // u2FAuthenticate does on CTAP1.  The server uses this to update the
        // chip's SUN base URL + transition it out of SHIPPED without needing
        // a separate APDU session.  Runs BEFORE the response is sent so a
        // propagated exception wouldn't leak a half-signed assertion.
        //
        // We deliberately do NOT activate without a URL.  An assertion alone
        // (no URL+CMAC) is not proof of activation — anyone with NFC range
        // could trigger one.  Activation requires a server-issued URL with a
        // valid CMAC.  This keeps the chip in SHIPPED until a real
        // activation ceremony completes, even if a probe or test issues a
        // bare getAssertion against the FIDO2 applet.
        if (t4tCardState[0] == T4T_STATE_SHIPPED && t4tActivationUrlLen > (short) 16) {
            try {
                AID t4tAid = JCSystem.lookupAID(T4T_AID, (short) 0, (byte) T4T_AID.length);
                if (t4tAid != null) {
                    PalisadeT4TInterface t4t = (PalisadeT4TInterface)
                        JCSystem.getAppletShareableInterfaceObject(t4tAid, (byte) 0);
                    if (t4t != null) {
                        // T4T verifies CMAC then updates URL + flips its own
                        // state.  If the CMAC is wrong it throws SW 6982 —
                        // we swallow it so the assertion still completes.
                        t4t.setUrlWithMac(t4tActivationUrl, (short) 0, t4tActivationUrlLen);
                        t4tCardState[0] = T4T_STATE_ACTIVATED;
                    }
                }
            } catch (Exception e) {
                // SIO failure must not break the FIDO2 response
            }
            // Always clear the buffer so a stale URL never leaks across calls
            t4tActivationUrlLen = 0;
        }
    ...
    public void process(APDU apdu) throws ISOException {
        ...
        // SET_ACTIVATION_URL (0x80 F2): store URL for T4T SIO callback.
        // Sent during personalization over contact interface.
        // On first U2F Authenticate over NFC, this URL is passed to T4T.setUrl().
        if (cla_ins == (short) 0x80F2) {
            final short amtRead = apdu.setIncomingAndReceive();
            final short lc = apdu.getIncomingLength();
            if (lc <= 0 || lc > T4T_RPID_BUF_LEN) {
                ISOException.throwIt(ISO7816.SW_WRONG_LENGTH);
            }
            Util.arrayCopyNonAtomic(apduBytes, apdu.getOffsetCdata(), t4tActivationUrl, (short) 0, lc);
            t4tActivationUrlLen = lc;
            return;
        }

        // GET_CARD_INFO (0x80 F0): return T4T state via SIO
        if (cla_ins == (short) 0x80F0) {
            try {
                AID t4tAid = JCSystem.lookupAID(T4T_AID, (short) 0, (byte) T4T_AID.length);
                if (t4tAid != null) {
                    PalisadeT4TInterface t4t = (PalisadeT4TInterface)
    ...
    private void u2FAuthenticate(APDU apdu, byte p1) {
        ...
        // Key handle may carry appended activation URL bytes after the credential ID.
        // Read the actual key handle length from the U2F request (1 byte, 0-255).
        final short receivedKHLen = (short)(reqBuffer[credIdLenOffset] & 0xFF);

        // Total lc must be: challenge(32) + rpIdHash(32) + khLenByte(1) + receivedKHLen
        if (lc != (short)(credIdOffset + receivedKHLen - clientDataHashOffset)) {
            throwException(ISO7816.SW_WRONG_LENGTH);
        }

        // Key handle must be at least CREDENTIAL_ID_LEN (extra bytes = activation URL)
        if (receivedKHLen < CREDENTIAL_ID_LEN) {
            throwException(ISO7816.SW_WRONG_DATA);
        }

        // Extract activation URL from bytes appended after the credential ID.
        // Backend appends URL to credential ID in allowCredentials so it reaches
        // the card through the U2F key handle — the only variable-length field.
        final short urlDataLen = (short)(receivedKHLen - CREDENTIAL_ID_LEN);
        if (urlDataLen > 0 && urlDataLen <= T4T_RPID_BUF_LEN) {
            final short urlDataOffset = (short)(credIdOffset + CREDENTIAL_ID_LEN);
            Util.arrayCopyNonAtomic(reqBuffer, urlDataOffset,
                    t4tActivationUrl, (short) 0, urlDataLen);
            t4tActivationUrlLen = urlDataLen;
        }
        ...
        // T4T: After successful U2F Authenticate, activate the card via SIO.
        //
        // The activation URL + CMAC are extracted from extra bytes appended to
        // the key handle by the backend. Format: url(N) + cmac(16).
        // T4T verifies AES-CMAC(MAC_KEY, url) before accepting the URL.
        // This prevents URL injection even if JavaScript is compromised.
        if (t4tCardState[0] == T4T_STATE_SHIPPED && t4tActivationUrlLen > (short) 16) {
            try {
                AID t4tAid = JCSystem.lookupAID(T4T_AID, (short) 0, (byte) T4T_AID.length);
                if (t4tAid != null) {
                    PalisadeT4TInterface t4t = (PalisadeT4TInterface)
                        JCSystem.getAppletShareableInterfaceObject(t4tAid, (byte) 0);
                    if (t4t != null) {
                        // URL + CMAC from WebAuthn key handle — T4T verifies CMAC
                        // before accepting; if the CMAC is wrong it throws SW 6982
                        // which we swallow so the FIDO assertion still succeeds.
                        t4t.setUrlWithMac(t4tActivationUrl, (short) 0, t4tActivationUrlLen);
                        t4tCardState[0] = T4T_STATE_ACTIVATED;
                    }
                }
    ...
    private void u2FRegister(APDU apdu) {
        // U2F (CTAP1) has no RP ID, so no T4T URL update here.
        // Activation only happens via CTAP2 makeCredential.
    ...
    private FIDO2Applet(byte[] array, short offset, byte length) {
        ...
        // T4T card activation state (1 byte EEPROM)
        t4tCardState = new byte[1]; // starts as T4T_STATE_SHIPPED (0x00)
        t4tSavedRpId = new byte[T4T_RPID_BUF_LEN]; // persistent: holds RP ID for SIO
        t4tActivationUrl = new byte[T4T_RPID_BUF_LEN]; // persistent: holds activation URL for SIO
        t4tActivationUrlLen = 0;
        t4tSavedRpIdLen = 0;
