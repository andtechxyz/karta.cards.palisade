...
public abstract class CannedCBOR {
    ...
    // karta.cards extension — server pushes <url || cmac(16)> as a byte
    // string to drive a SUN base-URL update on the T4T applet during
    // CTAP2 getAssertion.  Mirrors the url+CMAC tail that u2FAuthenticate
    // reads from the extended credential ID on CTAP1.
    static final byte[] KARTA_URL_EXTENSION_ID = {
            0x6B, 0x61, 0x72, 0x74, 0x61, 0x2D, 0x75, 0x72, 0x6C, // karta-url
    };
