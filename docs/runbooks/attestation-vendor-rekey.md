...
**Current status:** the attestation verifier is a stub at time of writing. Track 2 wires in the real verifier with the scaffolding this runbook describes. Read this as forward-looking documentation — the trust-anchor layout and rekey choreography will be in place once the verifier is real.

## 2. Where trust anchors live

Expected layout once Track 2 lands:
...
The verifier module loads both at startup into an in-memory cert pool. Anchor rotation follows the same dual-write pattern as [key-rotation.md §2](./key-rotation.md) — old and new anchors live side-by-side for a window so cards issued before the vendor rekey still verify.

Per-vendor config (which anchor file to load for which vendor) sits in `services/card-ops/src/attestation/config.ts` once the service is live. Don't hardcode anchor contents into code.
...
2. **Phase 1 — dual-anchor.** Concatenate the new bundle onto the existing Secrets Manager entry (old certs first, then new). Redeploy card-ops. The verifier now accepts chips issued under either anchor.
